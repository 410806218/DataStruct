...
int BinaryNodeTree<ItemType>::getHighestFullLevelHelper(BinaryNode<ItemType>* subTreePtr) const
{
	if(subTreePtr!=nullptr){
		if(subTreePtr->getLeftChildPtr()!=nullptr&&subTreePtr->getRightChildPtr()!=nullptr){
			int i= getHighestFullLevelHelper(subTreePtr->getRightChildPtr());
			int j= getHighestFullLevelHelper(subTreePtr->getLeftChildPtr());
			return 1+min(i,j);
		}
		else{
			return 1;
...
{	
		...
		if(subTreePtr!=nullptr){
		int i= getMaximumHelper(subTreePtr->getRightChildPtr());
		int j= getMaximumHelper(subTreePtr->getLeftChildPtr());
		
		return max(max(i,j),subTreePtr->getItem());
	}
...
{
		if(subTreePtr!=nullptr){
		int i= getNumberOfNodesHelper(subTreePtr->getRightChildPtr());
		int j= getNumberOfNodesHelper(subTreePtr->getLeftChildPtr());
