...
void BinarySearchTree<ItemType>::levelorder(void visit(BinaryNode<ItemType>&), BinaryNode<ItemType>* treePtr) const
{
    ...
    while (!nodeQueue.empty()) {
        ...
        if (tmpPtr->getLeftChildPtr() != nullptr)nodeQueue.push(tmpPtr->getLeftChildPtr());
        if (tmpPtr->getRightChildPtr() != nullptr)nodeQueue.push(tmpPtr->getRightChildPtr());
                                     ...
                                     bool& success)
{
    ...
    if (subTreePtr->getItem() == target&&subTreePtr->getCount()!=1) {
        subTreePtr->setCount(subTreePtr->getCount() - 1);
        success = true;
        return subTreePtr;
    }
	if ( subTreePtr->getItem() == target && subTreePtr->getCount() != 0) {
